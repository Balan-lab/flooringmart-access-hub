  ...
  head: () => ({
    meta: [
      { title: "Systems & Subscriptions — FlooringMart Access Manager" },
      {
        name: "description",
        content:
          "FlooringMart software systems, subscription costs, owners, renewals and review decisions.",
      },
      { property: "og:title", content: "Systems & Subscriptions — FlooringMart" },
      {
        property: "og:description",
        content: "Software inventory, spend, renewals and review decisions.",
      },
...
const ADD_SYSTEM_DRAFT_KEY = "flooringmart_access_hub_add_system_draft";
...
function SystemsPage() {
  ...
  const fields: Field[] = [
    ...
    },
    {
      ...
      options: DECISIONS.map((value) => ({
        value,
        label: DECISION_LABEL[value] ?? value,
      })),
    },
    {
  ...
  const columns: Column<System>[] = [
      ...
      cell: (r) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {r.vendor ?? "—"} · {r.category ?? "Uncategorised"}
          </p>
        </div>
      ),
    },
    {
      ...
      cell: (r) => (
        <div className="text-sm">
          <p>{r.business_owner ?? "—"}</p>
          <p className="text-xs text-muted-foreground">
            Technical: {r.technical_owner ?? "—"}
          </p>
        </div>
      ),
    },
    {
      key: "cost",
      header: "Cost",
      sortValue: (r) => Number(r.monthly_cost ?? 0),
      cell: (r) => (
        <div>
          <p>{currency(Number(r.monthly_cost ?? 0))}/mo</p>
          <p className="text-xs text-muted-foreground">
            {currency(Number(r.annual_cost ?? 0))}/yr
          </p>
        </div>
      ),
    },
    {
      key: "seats",
      header: "Seats",
      sortValue: (r) => r.paid_seats ?? 0,
      cell: (r) => `${r.active_users ?? 0} / ${r.paid_seats ?? 0}`,
    },
    {
      key: "renewal",
      header: "Renewal",
      sortValue: (r) => r.renewal_date ?? "",
      cell: (r) => formatDate(r.renewal_date),
    },
    {
      key: "decision",
      header: "Decision",
        ...
        <div className="flex justify-end gap-1">
          ...
          {canDelete ? (
            <ConfirmAction
              ...
              title={`Delete ${r.name}?`}
              description="This deletes the system record. Prefer changing its status to Cancelled so history remains intact."
              confirmLabel="Delete"
  ...
  const handleDialogOpenChange = (nextOpen: boolean) => {
    ...
    if (!nextOpen) {
      // Do NOT clear the Add System draft when the dialog is closed.
      // This allows the user to navigate away and continue later.
      setEditing(null);
    }
  };
  ...
  const handleSubmit = (values: Record<string, unknown>) => {
    save.mutate(
      ...
      {
        onSuccess: () => {
          // Only clear the draft after a successful new-system save.
          if (!editing) {
            sessionStorage.removeItem(ADD_SYSTEM_DRAFT_KEY);
          }
  ...
  return (
    <AppShell
      title="Systems & Subscriptions"
      description="Costs, renewals, ownership and review decisions for every registered system."
      actions={
        canWrite ? (
          <Button onClick={handleAddSystem}>
            <Plus className="mr-2 size-4" /> Add system
          </Button>
        ) : null
      }
    >
      <DataTable
        ...
        searchPlaceholder="Search systems, vendors or owners…"
        emptyMessage="No systems recorded."
      />

      <RecordDialog
        open={open}
        onOpenChange={handleDialogOpenChange}
        title={editing ? `Edit ${editing.name}` : "Add system"}
        description="Record decisions only. This app never cancels an external subscription automatically."
        fields={fields}
        initial={
          editing ?? {
            status: "active",
        ...
        draftKey={!editing ? ADD_SYSTEM_DRAFT_KEY : undefined}
